...
- **二步验证(2FA)**：支持TOTP标准(Google/Microsoft Authenticator等)，登录需验证6位动态验证码
...
### 二步验证(2FA)
1. **设置页面** → 二步验证区域 → 自动生成QR码
2. 用手机App扫描QR码（或手动输入密钥）
3. 输入App显示的6位验证码 → 点击"启用二步验证"
4. 下次登录需先输入用户名密码，再输入验证码
5. 支持的App：Google Authenticator、Microsoft Authenticator、1Password、Authy等
